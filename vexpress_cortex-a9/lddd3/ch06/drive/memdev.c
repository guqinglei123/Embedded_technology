...
static ssize_t mem_read(struct file *filp, char __user *buf, size_t size, loff_t *ppos)
{
  ...
  char * char_addr = mendev_addr->mem;
  // printk("mendev_addr is %x   %d\n", char_addr,count);
  /*读数据到用户空间*/
  if (copy_to_user(buf, mendev_addr->mem  + p, count))
  {
    ret = -EFAULT;
  }
  else
...
static ssize_t mem_write(struct file *filp, const char __user *buf, size_t size, loff_t *ppos)
{
  ...
  /*从用户空间写入数据*/
  if (copy_from_user(mendev_addr->mem + p, buf, count))
    ret = -EFAULT;
  else
  {
    *ppos += count;
...
static int memdev_init(void)
{
  ...
  for (index=0;index < memdev_nr_devs;index++)//这部分可以写成一个函数,配置cdev
  {
    ...
    int err = cdev_add(&mem_devpp->cdev, devt, 1);
    if (err)
